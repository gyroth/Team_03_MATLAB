...
%%
% 1i. Numerical Jacobian
% The following solves for the jacobian and then determines the velocity
% vector when using 30 degrees/s (pi/6 rad/s) for the instanteous joint
% velocities.

numerical_J = vpa(subs(J, symVariables, vals), 4)
inst_ang_vel = [pi/6; pi/6; pi/6];

% Units: [cm/s; cm/s; cm/s; rad/s; rad/s; rad/s]
vel_vector = vpa(numerical_J * inst_ang_vel, 4)